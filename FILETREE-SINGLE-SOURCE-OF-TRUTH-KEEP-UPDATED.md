# ⚠️ CRITICAL: SINGLE SOURCE OF TRUTH - MUST BE KEPT UPDATED ⚠️

## 🔴 MANDATORY INSTRUCTIONS FOR THIS FILE 🔴

**THIS FILE IS THE SINGLE SOURCE OF TRUTH FOR THE ENTIRE PLATFORM STRUCTURE.**

### ✅ REQUIRED ACTIONS:

1. **SAVE THIS FILE** - Never delete or lose this file. It maps the entire platform.
2. **UPDATE IMMEDIATELY** - Whenever you add, remove, or modify:
   - API endpoints
   - Dashboard modules
   - Components
   - Libraries
...
3. **TIMESTAMP EVERY UPDATE** - Update the timestamp at the bottom of this file with:
   - Current date and time in Indiana Eastern Time Zone
   - Format: `HH:MM:SS MMM DD, YYYY` (e.g., `14:30:45 Dec 03, 2025`)
   - Use: `TZ='America/Indiana/Indianapolis' date '+%H:%M:%S %b %d, %Y'`
4. **VERIFY ACCURACY** - Before committing changes, verify:
   - File counts match actual filesystem
   - Directory structures match reality
   - All new modules/components are documented
   - All removed items are removed from this doc
5. **NO DUPLICATES** - This is THE file. Don't create copies or variations.

### 🎯 PURPOSE:

This document exists because **this platform is a complex system** with:
- 241 API endpoints
- 19 dashboard modules
- 35 component modules
- 76 automation scripts
- 12 edge functions
- 47 database migrations
- Hundreds of files across multiple directories

**Without this map, navigating and maintaining this codebase becomes exponentially harder.**

### 📋 UPDATE CHECKLIST:

When making changes to the codebase, ask yourself:
- [ ] Did I add/remove any API routes? → Update `app/api/` section
- [ ] Did I add/remove any dashboard pages? → Update `app/(dashboard)/` section
- [ ] Did I add/remove any components? → Update `components/` section
- [ ] Did I add/remove any scripts? → Update `scripts/` section
- [ ] Did I add/remove any migrations? → Update `supabase/migrations/` section
- [ ] Did I add/remove any edge functions? → Update `supabase/functions/` section
- [ ] Did I update the timestamp at the bottom?
...
**Last Updated**: 11:55:33 Dec 03, 2025 (America/Indiana/Indianapolis)  
**Verified Against**: Actual codebase structure and file counts  
**Status**: ✅ Verified and accurate  
**File Name**: `FILETREE-SINGLE-SOURCE-OF-TRUTH-KEEP-UPDATED.md`  
**Purpose**: Single source of truth for entire platform structure

### 🔄 UPDATE HISTORY:

- **2025-12-03 11:47:34** - Initial verification and correction of all counts and structures
  - Corrected API endpoints: 58 → 241
  - Corrected dashboard modules: 24 → 19
  - Corrected components: 25+ → 35
  - Corrected Supabase functions: 11 → 12
  - Added missing modules and directories
  - Verified all file counts against actual filesystem

---

**⚠️ REMEMBER: UPDATE THIS FILE EVERY TIME YOU MODIFY THE CODEBASE STRUCTURE ⚠️**