// app/(dashboard)/jobs/page.tsx - FIXED VERSION (Part 1/2)
'use client'
...
function JobsPageContent() {
  ...
  const initialViewMode = searchParams.get('view') === 'kanban' ? 'kanban' : 'list'
  ...
  useEffect(() => {
    ...
    // CRITICAL FIX: Filter by account_id to prevent cross-account leakage
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        schema: 'public',
        table: 'jobs',
        filter: `account_id=eq.${accountId}`  // CRITICAL: Only this account's jobs
      }, (payload) => {
        console.log('[Jobs] Job inserted:', payload.new)
        // Add new job to the list
        setJobs(prevJobs => [payload.new as Job, ...prevJobs])
        // Show success notification (only for this account)
      ...
      .on('postgres_changes', {
        ...
        console.log('[Jobs] Job deleted:', payload.old)
      ...
      .subscribe((status) => {
        console.log('[Jobs] Subscription status:', status, 'for account:', accountId)
      })

    // Cleanup subscription on unmount
    return () => {
      supabase.removeChannel(channel)
      console.log('[Jobs] Unsubscribed from account:', accountId)
    }
  ...
  useEffect(() => {
    ...
    if (viewParam === 'kanban' && viewMode !== 'kanban') {
      setViewMode('kanban')
    } else if (viewParam !== 'kanban' && viewMode === 'kanban' && !viewParam) {
      setViewMode('list')
    }
  }, [searchParams, selectedJobId, openModal, viewMode])
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between p-4 border-b">
          <div>
            ...
            <p className="text-sm text-gray-500">Manage work orders and schedules</p>
          </div>
